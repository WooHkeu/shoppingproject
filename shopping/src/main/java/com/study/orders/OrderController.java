...
public class OrderController {
  ...
  public String create(
      ...
      @PathVariable String size, HttpSession session,Model model) {
    ...
    model.addAttribute("qty",qty);//수량(order detail테이블에 저장, 총금액)
    model.addAttribute("size",size);//size(order detail테이블에 저장)
  ...
  public String cart(
    ...
    model.addAttribute("list",list);//상품목록
    model.addAttribute("mdto",mdto);//회원정보
    model.addAttribute("qtys",qty);//수량들(order detail테이블에 저장, 총금액)
    model.addAttribute("size",size);//size들(order detail테이블에 저장)
